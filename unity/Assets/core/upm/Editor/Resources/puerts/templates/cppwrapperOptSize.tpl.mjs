...
*/
import { ELSE, ENDIF, FOR, IF, default as t } from "./tte.mjs";
...
function genBridge(bridgeInfo) {
    ...
    return t`
    static void b_${bridgeInfo.Signature}(void* target, ${parameterSignatures.map((S, i) => `${CODE_SNIPPETS.SToCPPType(S)} p${i}`).map(s => `${s}, 
    `).join('')}${CODE_SNIPPETS.SToCPPType(bridgeInfo.ReturnSignature)} * __pret, void* method) {
